...
Devices::Devices(VkInstance *instance,
...
void Devices::pickPhysicalDevice() {
  ...
  for (const auto &currDevice : devices) {
    if (!isDeviceSuitable(currDevice))
      continue;

    int score = rateDeviceSuitability(currDevice);
    candidates.insert(std::make_pair(score, currDevice));
  }
...
bool Devices::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  std::cout << "isDeviceSuitable: " << result << "\n";
  return result;
}
...
Devices::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR Devices::chooseSwapSurfaceFormat(
...
VkPresentModeKHR Devices::chooseSwapPresentMode(
...
VkExtent2D
Devices::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
